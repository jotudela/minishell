...
/*   Updated: 2025/04/01 14:34:02 by jotudela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*/
void	initialize_shell(char **envp, struct termios *attrs, int ac, ...)
{
	char	*tmp;
	int		value;
...
*/
int	main(int ac, char **__, char **envp)
{
	char			*command_line;
	struct termios	attrs[3];

	if (ac != 1)
		return (ft_putendl_fd("No interavtive mode.", 2), 0);
	initialize_shell(envp, attrs, ac, __);
